...
const workflowController = {
    ...
    saveWorkflowGraph: async (req, res) => {
    ...
    archiveWorkflow: async (req, res) => {
        const workflowId = req.params?.workflowId;
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required");
        const workflow = await workflowService.archiveWorkflow({ workflowId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    restoreWorkflow: async (req, res) => {
        const workflowId = req.params?.workflowId;
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required");
        const workflow = await workflowService.restoreWorkflow({ workflowId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteWorkflow: async (req, res) => {
        ...
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required");
        const workflow = await workflowService.deleteWorkflow({ workflowId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    createStages: async (req, res) => {
        ...
        const workflow = await workflowService.createStages({ workflowId }, stagesData, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    updateStage: async (req, res) => {
        ...
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required");
        const updatedStage = await workflowService.updateStages({ workflowId }, data, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteStage: async (req, res) => {
        ...
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required");
        const deletedStage = await workflowService.deleteStages({ workflowId }, data, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    getStage: async (req, res) => {
        ...
        const stages = await workflowService.getStagesByWorkflowIdandStageId({ workflowId }, { stageId });
        sendResponse({
            res,
            statusCode: HTTPSTATUS.OK,
            success: true,
    ...
    /*                                   TRANSISTION                              */
    /* -------------------------------------------------------------------------- */
    createTransitions: async (req, res) => {
        const workflowId = req?.params?.workflowId;
        if (!workflowId) {
            throw new BadRequestException("Workflow Id is required.");
        }
        const transitionsData = req.body;
        const createdTransitions = await workflowService.createTransitions({ workflowId }, transitionsData, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    getTransition: async (req, res) => {
        ...
        const transition = await workflowService.getTransitionById({ workflowId }, transitionId);
        sendResponse({
            res,
            statusCode: HTTPSTATUS.OK,
            success: true,
    ...
    updateTransition: async (req, res) => {
        ...
        const updatedTransition = await workflowService.updateTransition({ workflowId }, transitionId, data, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteTransitions: async (req, res) => {
        const workflowId = req.params?.workflowId;
        const transitionId = req.params.transitionId;
        if (!workflowId)
            throw new BadRequestException("Workflow Id is required.");
        ...
        const deletedTransitions = await workflowService.deleteTransition({ workflowId }, transitionId, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    start: async (req, res) => {
        ...
        const instance = await workflowRuntimeService.startInstance(workflowId, { resourceId, resourceType }, {
            actorId: req.user.id,
    ...
    transition: async (req, res) => {
        ...
        const result = await workflowRuntimeService.executeTransition(instanceId, { transitionId, action, comment }, {
            actorId: req.user.id,
            ipAddress: req.ip ?? null,
            userAgent: req.get("user-agent") ?? null,
            performedBy: "USER",
    ...
    getAvailableActions: async (req, res) => {
        ...
        const actions = await workflowRuntimeService.getAvailableActions(instanceId, req.user.id);
        return sendResponse({
            res,
            statusCode: HTTPSTATUS.OK,
            success: true,
    ...
    getVisualizer: async (req, res) => {
        const { workflowId } = req.params;
        if (!workflowId) {
            throw new BadRequestException("workflowId is required");
        }
        const visualizer = await workflowVisualizerService.getWorkflowVisualizer(workflowId);
        return sendResponse({
            res,
            statusCode: HTTPSTATUS.OK,
            success: true,
