import { HTTPSTATUS, HttpStatusMessageMap, } from "../config/http.config.js";
/**
 * Send a consistent API response.
 * - Will not send a body for 204 No Content.
 * - Sets X-Request-Id header when available.
 */
export const sendResponse = ({ res, statusCode = HTTPSTATUS.OK, success, message, data, meta, requestId, }) => {
    ...
    const resolvedRequestId = requestId ?? (res.locals && res.locals.requestId) ?? res.getHeader("X-Request-Id") ?? undefined;
