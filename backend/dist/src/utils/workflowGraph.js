...
export const validateWorkflowGraph = async (workflowId, tx) => {
    ...
    for (const stage of stages) {
        if (stage.isFinal) {
            const outgoing = transitions.filter((t) => t.fromStageId === stage.id &&
                t.transitionType !== "SEND_BACK" &&
                t.transitionType !== "REVIEW" &&
                t.transitionType !== "AUTO");
            if (outgoing.length > 0) {
                throw new BadRequestException(`Final stage "${stage.name}" cannot have outgoing transitions`);
            }
        }
    }
    ...
    for (const stage of stages) {
        if (!visited.has(stage.id) && dfs(stage.id)) {
            throw new BadRequestException("Workflow contains circular forward transitions");
        }
    }
    /* ---------- REACHABILITY ---------- */
    ...
    if (unreachable.length) {
        throw new BadRequestException(`Unreachable stages: ${unreachable.map((s) => s.name).join(", ")}`);
    }
    ...
    for (const stage of stages) {
        if (!stage.isFinal) {
            const outgoing = transitions.filter((t) => t.fromStageId === stage.id &&
                ...
                t.transitionType !== "AUTO");
            if (!outgoing.length) {
                throw new BadRequestException(`Stage "${stage.name}" has no outgoing transitions`);
            }
        }
    }
    return true;
};
//# sourceMappingURL=workflowGraph.js.map