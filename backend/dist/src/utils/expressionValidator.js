...
export function validateExpression(expr, opts) {
    ...
    while ((match = tokenRegex.exec(expr)) !== null) {
        ...
        if (/^[A-Za-z_][A-Za-z0-9_\.]*$/.test(token)) {
            ...
            if (["true", "false", "null"].includes(token.toLowerCase()))
                continue;
        ...
        if (allowedOperators.has(token) || token === "(" || token === ")")
            continue;
    ...
    for (const b of blacklist) {
        if (expr.includes(b))
            return { valid: false, error: `disallowed substring: ${b}` };
    }
    return { valid: true };
}
