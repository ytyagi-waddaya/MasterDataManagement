...
const colorStatus = (status) => {
    ...
    if (status >= 400)
        return chalk.redBright(String(status));
    if (status >= 300)
...
function isHttpMeta(obj) {
    if (!obj || typeof obj !== "object")
        return false;
    const o = obj;
    return (o.component === "http" &&
...
const createPrintf = (stripAnsiCodes = false) => printf(({ level, message, timestamp: ts, stack, ...meta }) => {
    const levelKey = (level || "info").toUpperCase();
    const tsVal = typeof ts === "string" ? ts : new Date().toISOString();
    const colorFn = levelColors[levelKey] ?? ((s) => s);
    ...
    // Generic logs
    const metaEntries = meta && Object.keys(meta).length
        ? " " +
            Object.entries(meta)
...
export function httpLogger() {
    return morgan((tokens, req, res) => {
        try {
            ...
            const responseTimeRaw = typeof responseTimeToken === "function" ? responseTimeToken(req, res) : undefined;
            ...
            const method = typeof methodToken === "function" ? methodToken(req, res) ?? req.method : req.method;
            const url = typeof urlToken === "function" ? urlToken(req, res) ?? req.originalUrl : req.originalUrl;
            const statusFromToken = typeof statusToken === "function" ? statusToken(req, res) : undefined;
            const status = Number(statusFromToken ?? res.statusCode ?? 0) || 0;
            const ip = typeof remoteAddrToken === "function"
