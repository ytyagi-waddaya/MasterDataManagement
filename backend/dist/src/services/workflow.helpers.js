import { ApprovalStatus, HistoryAction, } from "../../prisma/generated/client.js";
import { ForbiddenException } from "../utils/appError.js";
/* ======================================================
   PERMISSION CHECK
====================================================== */
export async function enforceTransitionPermission(userId, instance, transition, tx) {
...
====================================================== */
export function resolveApprovalStrategy(strategy, approvals, expectedCount) {
    ...
    switch (strategy) {
        case "ALL":
            if (rejected > 0)
                return "REJECTED";
            if (approved === expectedCount)
                return "APPROVED";
            return "PENDING";
        case "ANY":
            if (approved > 0)
                return "APPROVED";
            if (rejected === expectedCount)
                return "REJECTED";
            return "PENDING";
        case "MAJORITY":
            if (approved > expectedCount / 2)
                return "APPROVED";
            if (rejected >= Math.ceil(expectedCount / 2))
                return "REJECTED";
            return "PENDING";
    }
}
...
====================================================== */
export function getCurrentApprovalLevel(config, approvals) {
    const levels = [...config.levels].sort((a, b) => a.order - b.order);
    for (const level of levels) {
        const levelApprovals = approvals.filter((a) => a.levelOrder === level.order);
        // ❌ Any rejection at this level stops workflow
        const rejected = levelApprovals.some((a) => a.status === ApprovalStatus.REJECTED);
        if (rejected)
            return null;
        const expected = (level.userIds?.length ?? 0) + (level.roleIds?.length ?? 0);
...
/* ======================================================
   MOVE TO STAGE
====================================================== */
export async function moveToStage(tx, instance, transition, meta, comment) {
