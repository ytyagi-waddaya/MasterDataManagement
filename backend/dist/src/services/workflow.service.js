...
import { createWorkflowDefinitionSchema, stageIdSchema, updateWorkflowSchema, workflowFilterSchema, workflowIdSchema, } from "../types/workflow.types.js";
import { generateStageCode, generateWorkflowCodeAndVersion, } from "../utils/workflowCode.js";
...
const WorkflowService = {
    ...
    //   });
    // },
    saveWorkflowGraph: async (workflowId, data, meta) => {
        const actorId = meta?.actorId;
        if (!actorId)
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const t of data.transitions) {
                ...
                if (transitionType === "AUTO" && triggerStrategy !== "SYSTEM_ONLY") {
                    throw new BadRequestException("AUTO transitions must use SYSTEM_ONLY trigger strategy");
                }
                if (triggerStrategy === "SYSTEM_ONLY" &&
                    transitionType !== "AUTO") {
                    throw new BadRequestException("SYSTEM_ONLY trigger strategy is only valid for AUTO transitions");
                }
                if (triggerStrategy === "APPROVER_ONLY" &&
                    transitionType !== "APPROVAL") {
                    throw new BadRequestException("APPROVER_ONLY trigger strategy is only valid for APPROVAL transitions");
                }
                /* ---------- REVIEW ---------- */
                if (transitionType === "REVIEW" && fromStageId !== toStageId) {
                    throw new BadRequestException("REVIEW transition must be a self-loop");
                }
                /* ---------- AUTO ---------- */
                if (transitionType === "AUTO" &&
                    (t.allowedRoleIds?.length || t.allowedUserIds?.length)) {
                    throw new BadRequestException("AUTO transitions cannot have users or roles");
                }
                /* ---------- APPROVAL ---------- */
                if (transitionType === "APPROVAL") {
                    if (!t.approvalStrategy) {
                        throw new BadRequestException("approvalStrategy is required for APPROVAL transitions");
                    }
                    if (!t.approvalConfig) {
                        throw new BadRequestException("approvalConfig is required for APPROVAL transitions");
                    }
                    const config = t.approvalConfig;
                    if (config.mode === "SEQUENTIAL") {
                        if (!Array.isArray(config.levels) || !config.levels.length) {
                            throw new BadRequestException("Sequential approval requires approval levels");
                        }
                        const orders = config.levels.map((l) => l.order);
                        if (orders.length !== new Set(orders).size) {
                            throw new BadRequestException("Approval levels must have unique order");
                        }
                    }
                    if (config.mode === "PARALLEL") {
                        const level = config.levels?.[0];
                        const count = (level?.userIds?.length ?? 0) +
                            (level?.roleIds?.length ?? 0);
                        if (!count) {
                            throw new BadRequestException("Parallel approval requires users or roles");
                        }
                    }
                }
                if (transitionType !== "APPROVAL" && t.approvalConfig) {
                    throw new BadRequestException("approvalConfig is only allowed for APPROVAL transitions");
                }
                /* ---------- CREATE TRANSITION ---------- */
                const transitionData = {
                    ...
                    transitionType,
                    triggerStrategy,
                    approvalStrategy: transitionType === "APPROVAL"
                    ...
                    approvalConfig: transitionType === "APPROVAL"
    ...
    createWorkflow: async (data, meta) => {
        ...
        if (!resourceExists) {
            throw new BadRequestException("Invalid resourceId — resource does not exist.");
        }
        const { code, version } = await generateWorkflowCodeAndVersion(validatedData.name);
        const duplicate = await workflowRepository.findByCodeAndVersion(code, version);
        if (duplicate) {
            throw new BadRequestException("Workflow with the same version already exists. Try again.");
        }
        const workflow = await workflowRepository.createWorkflow({
    ...
    publishWorkflow: async (workflowId, meta) => {
        const actorId = meta?.actorId;
        if (!actorId)
            throw new BadRequestException("Actor ID is required");
        const workflow = await prisma.workflowDefinition.findUnique({
    ...
    updateWorkflow: async (id, data, meta) => {
        ...
        if (validatedData.name && validatedData.name !== existing.name) {
            const duplicate = await workflowRepository.isDuplicateName(validatedData.name, parsedId.workflowId);
            if (duplicate) {
        ...
        if (validatedData.resourceId) {
            ...
            if (!resourceExists) {
                throw new BadRequestException("Invalid resourceId — resource not found.");
            }
        }
        ...
        const updated = await prisma.$transaction(async (tx) => {
            ...
            if (validatedData.name && validatedData.name !== existing.name) {
                const { code, version } = await generateWorkflowCodeAndVersion(validatedData.name, tx);
    ...
    archiveWorkflow: async ({ workflowId }, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const existing = await workflowRepository.getWorkflowById(parsedId);
        if (!existing)
            throw new NotFoundException("Workflow not found");
    ...
    restoreWorkflow: async ({ workflowId }, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const existing = await workflowRepository.getWorkflowById(parsedId);
        if (!existing)
            throw new NotFoundException("Workflow not found");
    ...
    deleteWorkflow: async ({ workflowId }, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const existing = await workflowRepository.getWorkflowById(parsedId);
        if (!existing)
            throw new NotFoundException("Workflow not found");
        ...
        if (hasInstances) {
            throw new BadRequestException("Cannot delete workflow because instances exist. Archive instead.");
        }
    ...
    /*                                   STAGES                                   */
    /* -------------------------------------------------------------------------- */
    createStages: async ({ workflowId }, data, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!parsedId)
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const stageInput of data) {
                ...
                const duplicateCode = await workflowRepository.findByCode(workflowId, code, tx);
                if (duplicateCode) {
                    throw new BadRequestException(`Stage with code "${code}" already exists`);
                }
                // Create the Stage
                const stage = await workflowRepository.createStage({ ...stageInput, code }, tx);
    ...
    updateStages: async ({ workflowId }, stagesData, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const workflowExists = await workflowRepository.getWorkflowById(parsedId);
        if (!workflowExists)
            throw new NotFoundException("Workflow not found");
        if (!stagesData)
            throw new BadRequestException("No stage data provided");
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const input of stagesArray) {
                ...
                if (input.name && input.name !== stage.name) {
                    ...
                    if (nameExists) {
                        throw new BadRequestException(`Stage name "${input.name}" already exists in this workflow`);
                    }
                }
                /** ------------------------------------------------------------------
                ...
                if (input.name && input.name !== stage.name) {
                    ...
                    const codeExists = await workflowRepository.findByCode(workflowId, newCode, tx);
                    if (codeExists) {
                        throw new BadRequestException(`Generated code "${newCode}" already exists. Try different stage name.`);
                    }
                }
                /** ------------------------------------------------------------------
                 ...
                 * 6. Apply update
                 * ------------------------------------------------------------------ */
                const updated = await workflowRepository.updateStage(stage.id, updatePayload, tx);
    ...
    deleteStages: async ({ workflowId }, stagesData, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const workflowExists = await workflowRepository.getWorkflowById(parsedId);
        if (!workflowExists)
            throw new NotFoundException("Workflow not found");
        ...
        return prisma.$transaction(async (tx) => {
            ...
            for (const { stageId } of stagesArray) {
                ...
                if (transitionsExist) {
                    throw new BadRequestException(`Stage "${stage.name}" cannot be deleted because transitions depend on it.`);
                }
                /** ------------------------------------------------------------------
                ...
                if (instanceExists) {
                    throw new BadRequestException(`Stage "${stage.name}" cannot be deleted because workflow instances refer to it.`);
                }
                /** ------------------------------------------------------------------
                 * 3. Perform deletion
                 * ------------------------------------------------------------------ */
    ...
    getStagesByWorkflowId: async ({ workflowId }) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const stages = await workflowRepository.findByWorkflowId(parsedId);
        return stages;
    },
    getStagesByWorkflowIdandStageId: async ({ workflowId }, { stageId }) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        const parsedStageId = stageIdSchema.parse({ stageId });
        const stages = await workflowRepository.getStagesByWorkflowIdandStageId(parsedId, parsedStageId);
        return stages;
    },
    // /* -------------------------------------------------------------------------- */
    // /*                              TRANSITION                                    */
    ...
    //     });
    //   },
    createTransitions: async ({ workflowId }, transitionsData, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!workflowId)
            throw new BadRequestException("workflowId is required.");
        if (!Array.isArray(transitionsData) || transitionsData.length === 0) {
        ...
        return prisma.$transaction(async (tx) => {
            const createdTransitions = [];
            for (const t of transitionsData) {
                const { fromStageId, toStageId, label, allowedUserIds, allowedRoleIds, } = t;
                // Validate stage existence
                const fromStage = await workflowRepository.findByStageId(fromStageId, tx);
                if (!fromStage)
                    throw new BadRequestException(`fromStageId "${fromStageId}" does not exist`);
                const toStage = await workflowRepository.findByStageId(toStageId, tx);
                if (!toStage)
                    throw new BadRequestException(`toStageId "${toStageId}" does not exist`);
                // Rule: cannot transition to same stage
                if (fromStageId === toStageId)
                    throw new BadRequestException("A transition cannot point to the same stage");
                // Rule: cannot create outgoing transitions from final stage
                if (fromStage.isFinal) {
                    throw new BadRequestException(`Cannot create transition from final stage "${fromStage.name}".`);
                }
                // Rule: cannot create incoming transitions into initial stage
                if (toStage.isInitial) {
                    throw new BadRequestException(`Cannot create transition into initial stage "${toStage.name}".`);
                }
                // Prevent duplicate transitions
                const duplicate = await tx.workflowTransition.findFirst({
                    where: { workflowId, fromStageId, toStageId },
                ...
                if (duplicate) {
                    throw new BadRequestException(`Transition from "${fromStage.name}" to "${toStage.name}" already exists`);
                }
                // Create transition
                const created = await workflowRepository.createTransition({ ...t, workflowId }, tx);
                createdTransitions.push(created);
                // Save allowed roles
                if (allowedRoleIds?.length) {
                    await tx.workflowTransitionAllowedRole.createMany({
    ...
    getTransitions: async ({ workflowId }) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!workflowId)
            throw new BadRequestException("workflowId is required.");
        const workflowExists = await workflowRepository.getWorkflowById(parsedId);
    ...
    getTransitionById: async ({ workflowId }, transitionId) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!parsedId)
            throw new BadRequestException("workflowId is required.");
        const workflowExists = await workflowRepository.getWorkflowById(parsedId);
    ...
    //   });
    // },
    updateTransition: async ({ workflowId }, transitionId, data, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!parsedId)
            throw new BadRequestException("workflowId is required.");
        const workflowExists = await workflowRepository.getWorkflowById(parsedId);
        ...
        return prisma.$transaction(async (tx) => {
             ...
             * ----------------------------------------------------------- */
            if (data.fromStageId) {
                const from = await workflowRepository.findByStageId(data.fromStageId, tx);
                if (!from)
                    throw new BadRequestException(`fromStageId "${data.fromStageId}" does not exist`);
            }
            if (data.toStageId) {
                const to = await workflowRepository.findByStageId(data.toStageId, tx);
                if (!to)
                    throw new BadRequestException(`toStageId "${data.toStageId}" does not exist`);
            }
            if (data.fromStageId &&
                data.toStageId &&
                data.fromStageId === data.toStageId) {
            ...
            if (Array.isArray(data.allowedRoleIds)) {
                ...
                if (data.allowedRoleIds.length > 0) {
                    await tx.workflowTransitionAllowedRole.createMany({
                        data: data.allowedRoleIds.map((roleId) => ({
                            transitionId,
                            roleId,
                        })),
                    });
            ...
            if (Array.isArray(data.allowedUserIds)) {
                ...
                if (data.allowedUserIds.length > 0) {
                    await tx.workflowTransitionAllowedUser.createMany({
                        data: data.allowedUserIds.map((userId) => ({
                            transitionId,
                            userId,
                        })),
                    });
             ...
             * 5. UPDATE TRANSITION
             * ----------------------------------------------------------- */
            const updated = await workflowRepository.updateTransition(transitionId, safeData, tx);
            /* -------------------------------------------------------------
    ...
    deleteTransition: async ({ workflowId }, transitionId, meta) => {
        const parsedId = workflowIdSchema.parse({ workflowId });
        if (!transitionId) {
        ...
        return prisma.$transaction(async (tx) => {
            ...
            const deleted = await workflowRepository.deleteTransition(transitionId, tx);
            if (meta?.actorId) {
                await createAuditLog({
                    userId: meta.actorId,
                    entity: AuditEntity.WORKFLOW,
    ...
    //   });
    // },
    closeInstance: async (workflowId, instanceId, meta) => {
        const workflow = await workflowRepository.getWorkflowWithStages(workflowId);
        if (!workflow) {
            throw new NotFoundException("Workflow not found");
        }
        ...
        return prisma.$transaction(async (tx) => {
            const now = new Date();
            const closed = await workflowRepository.closeInstance(instanceId, now, tx);
    ...
    getInstance: async (workflowId, instanceId) => {
        const workflow = await workflowRepository.getWorkflowWithStages(workflowId);
        if (!workflow) {
            throw new NotFoundException("Workflow not found");
        }
    ...
    listInstancesByWorkflow: async (workflowId, options) => {
...
async function resolveUsersFromRoles(roleIds) {
    if (!roleIds.length)
        return [];
...
async function getNotificationTargetsForStage(workflowId, stageId) {
    ...
    if (!nextTransitions.length)
        return [];
    const directUsers = nextTransitions.flatMap((t) => t.allowedUsers.map((u) => u.userId));
    const roleIds = nextTransitions.flatMap((t) => t.allowedRoles.map((r) => r.roleId));
