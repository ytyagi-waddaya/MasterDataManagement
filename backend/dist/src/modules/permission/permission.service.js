...
import { updatePermissionSchema, updatePermissionConditionSchema, permissionIdSchema, permissionIdsSchema, permissionFilterSchema, } from "./dto/permission.dto.js";
import { NotFoundException, BadRequestException } from "../../utils/appError.js";
import { createAuditLog } from "../../utils/auditLog.js";
import { prisma } from "../../lib/prisma.js";
import { resourceIdSchema } from "../resource/dto/resource.dto.js";
...
const permissionService = {
    generatePermissions: async () => {
        ...
        for (const action of actions) {
            for (const resource of resources) {
                ...
                const description = resourceName.toLowerCase().startsWith(actionName.toLowerCase())
    ...
    updatePermissionById: async (id, data, meta) => {
        ...
        if (validatedData.name && validatedData.name !== existing.name) {
            const duplicate = await permissionRepository.isDuplicateName(validatedData.name, parsedId.permissionId);
            if (duplicate)
    ...
    updatePermissionCondition: async (id, input, meta) => {
        const parsedId = permissionIdSchema.parse(id);
    ...
    archivePermission: async ({ permissionId }, meta) => {
        const parsedId = permissionIdSchema.parse({ permissionId });
        const existing = await permissionRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Permission not found");
    ...
    restorePermission: async ({ permissionId }, meta) => {
        const parsedId = permissionIdSchema.parse({ permissionId });
        const existing = await permissionRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Permission not found");
    ...
    deletePermission: async ({ permissionId }, meta) => {
        const parsedId = permissionIdSchema.parse({ permissionId });
        const existing = await permissionRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Permission not found");
    ...
    archivePermissions: async (ids, meta) => {
        const { permissionIds } = permissionIdsSchema.parse(ids);
        const existingPermissions = await permissionRepository.findManyByIds(permissionIds);
        const activePermissions = existingPermissions.filter((p) => !p.deletedAt);
        if (activePermissions.length === 0) {
            throw new BadRequestException("No active permissions found for the provided IDs.");
        }
        const nonSystemActive = activePermissions.filter((p) => !p.isSystem);
        if (nonSystemActive.length === 0) {
            throw new BadRequestException("All selected permissions are system permissions and cannot be archived.");
        }
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await Promise.all(updatedRecords.map((updated) => tx.auditLog.create({
                data: {
                    ...
                    before: safe(existingPermissions.find((p) => p.id === updated.id)),
                    after: safe({ ...updated }),
                    ipAddress: meta?.ipAddress ?? null,
                    userAgent: meta?.userAgent ?? null,
                    performedBy: meta?.performedBy ?? PerformedByType.USER,
    ...
    restorePermissions: async (ids, meta) => {
        const { permissionIds } = permissionIdsSchema.parse(ids);
        const existingPermissions = await permissionRepository.findManyByIds(permissionIds);
    ...
    deletePermissions: async (ids, meta) => {
        const { permissionIds } = permissionIdsSchema.parse(ids);
        const existingPermissions = await permissionRepository.findManyByIds(permissionIds);
        if (existingPermissions.length === 0) {
            throw new BadRequestException("No permissions found for the provided IDs.");
        }
        ...
        if (deletable.length === 0) {
            throw new BadRequestException("All selected permissions are system permissions and cannot be deleted.");
        }
    ...
    getPermissionsByResource: async ({ resourceId }) => {
    ...
    getPermissionsByAction: async ({ actionId }) => {
