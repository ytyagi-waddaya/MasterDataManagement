...
const permissionController = {
    ...
    getPermissionsByAction: async (req, res) => {
    ...
    updatePermissionCondition: async (req, res) => {
        const { permissionId } = req.params;
        if (!permissionId)
            throw new BadRequestException("Permission ID is required");
        const permission = await permissionService.updatePermissionCondition({ permissionId }, req.body, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    archivePermission: async (req, res) => {
        const permissionId = req.params?.permissionId;
        if (!permissionId) {
            throw new BadRequestException("permission Id is required");
        }
        const archived = await permissionService.archivePermission({ permissionId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
        });
    ...
    restorePermission: async (req, res) => {
        const permissionId = req.params?.permissionId;
        if (!permissionId) {
            throw new BadRequestException("permission Id is required");
        }
        const restored = await permissionService.restorePermission({ permissionId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
        });
    ...
    archivePermissions: async (req, res) => {
    ...
    assignPermissionsToRole: async (req, res) => {
        ...
        if (!roleId)
            throw new BadRequestException("roleId required");
