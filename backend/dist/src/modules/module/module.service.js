...
const moduleService = {
    createModule: async (data, meta) => {
        const validatedData = createModuleSchema.parse(data);
        const key = generateKey(validatedData.name);
        const existingByName = await moduleRepository.findByNameAndTenant(validatedData.name);
        if (existingByName) {
            throw new BadRequestException(`Module "${validatedData.name}" already exists.`);
        }
    ...
    updateModuleById: async (id, data, meta) => {
        ...
        if (validatedData.name && validatedData.name !== existing.name) {
            const duplicate = await moduleRepository.isDuplicateName(validatedData.name, parsedId.moduleId);
            if (duplicate)
                throw new BadRequestException("module name already exists.");
        }
        let dataToUpdate = {};
    ...
    archiveModule: async ({ moduleId }, meta) => {
        const parsedId = moduleIdSchema.parse({ moduleId });
        const existing = await moduleRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Module not found.");
    ...
    restoreModule: async ({ moduleId }, meta) => {
        const parsedId = moduleIdSchema.parse({ moduleId });
        const existing = await moduleRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Module not found.");
    ...
    deleteModule: async ({ moduleId }, meta) => {
        const parsedId = moduleIdSchema.parse({ moduleId });
        const existing = await moduleRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Module not found.");
    ...
    archiveModules: async (ids, meta) => {
        ...
        if (activeModules.length === 0) {
            throw new BadRequestException("No active modules found for the provided IDs.");
        }
        const nonSystemActive = activeModules.filter((m) => !m.isSystem);
        if (nonSystemActive.length === 0) {
            throw new BadRequestException("All selected modules are system modules and cannot be archived.");
        }
    ...
    restoreModules: async (ids, meta) => {
        const { moduleIds } = moduleIdsSchema.parse(ids);
        const existingModules = await moduleRepository.findManyByIds(moduleIds);
        const inactiveModules = existingModules.filter((m) => m.deletedAt);
        if (inactiveModules.length === 0) {
            throw new BadRequestException("No archived modules found for the provided IDs.");
        }
    ...
    deleteModules: async (ids, meta) => {
        ...
        if (deletable.length === 0) {
            throw new BadRequestException("All selected modules are system modules and cannot be deleted.");
        }
