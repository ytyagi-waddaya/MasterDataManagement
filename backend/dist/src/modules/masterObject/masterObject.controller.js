...
const masterObjectController = {
    ...
    getMasterObjects: async (req, res) => {
    ...
    updateMasterObject: async (req, res) => {
        const masterObjectId = req.params?.masterObjectId;
        if (!masterObjectId)
            throw new BadRequestException("MasterObject ID is required");
        const data = req.body;
        const masterObject = await masterObjectService.updateMasterObject({ masterObjectId }, data, {
    ...
    archiveMasterObject: async (req, res) => {
        const masterObjectId = req.params?.masterObjectId;
        if (!masterObjectId)
            throw new BadRequestException("MasterObject ID is required");
        const masterObject = await masterObjectService.archivemasterObject({ masterObjectId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    restoreMasterObject: async (req, res) => {
        const masterObjectId = req.params?.masterObjectId;
        if (!masterObjectId)
            throw new BadRequestException("MasterObject ID is required");
        const masterObject = await masterObjectService.restoremasterObject({ masterObjectId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteMasterObject: async (req, res) => {
        const masterObjectId = req.params?.masterObjectId;
        if (!masterObjectId)
            throw new BadRequestException("MasterObject ID is required");
        const result = await masterObjectService.deletemasterObject({ masterObjectId }, {
            actorId: req.user?.id ?? null,
            performedBy: PerformedByType.USER,
            userAgent: req.get("user-agent") ?? null,
            ipAddress: req.ip,
...
//# sourceMappingURL=masterObject.controller.js.map