...
import { AuditAction, AuditEntity, PerformedByType, } from "../../../prisma/generated/enums.js";
...
const rolePermissionService = {
    ...
    //   });
    // },
    updateAccessLevel: async ({ roleId, permissionId, input, meta, }) => {
        /* ------------------------------
        ...
        if (input.accessLevel === "CONDITIONAL") {
            ...
            try {
                ...
                throw new BadRequestException(`Invalid conditions: ${err?.message ?? "invalid"}`);
            }
    ...
    grantAllModulePermissions: async ({ roleId, moduleId, }) => {
        // Ensure role exists
        const role = await prisma.role.findUnique({ where: { id: roleId } });
        if (!role)
            throw new NotFoundException("Role not found");
        // Get all permission IDs under this module
        const permissionIds = await rolePermissionRepository.getModulePermissionIds(moduleId);
        if (permissionIds.length === 0)
            throw new NotFoundException("No permissions found for this module");
        return prisma.$transaction(async (tx) => {
            const assignments = await Promise.all(permissionIds.map((pid) => tx.rolePermission.upsert({
    ...
    revokeAllModulePermissions: async ({ roleId, moduleId, }) => {
        // Ensure role exists
        const role = await prisma.role.findUnique({ where: { id: roleId } });
        if (!role)
            throw new NotFoundException("Role not found");
