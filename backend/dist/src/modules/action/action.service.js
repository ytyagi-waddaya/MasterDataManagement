...
import { createActionSchema, updateActionSchema, actionIdSchema, actionIdsSchema, actionFilterSchema, } from "./dto/action.dto.js";
import { generateKey } from "../../common/utils/generate-key.js";
import { OutboxService } from "../../outbox/outbox.service.js";
import { prisma } from "../../lib/prisma.js";
import { safe } from "../../common/utils/sanitize.js";
const actionsService = {
    createAction: async (data, meta) => {
        const validatedData = createActionSchema.parse(data);
        const key = generateKey(validatedData.name);
        const existing = await actionsRepository.findByNameAndTenant(validatedData.name);
        if (existing) {
            throw new BadRequestException(`Action "${validatedData.name}" already exists.`);
        }
        const toCreate = {
    ...
    updateActionById: async (id, data, meta) => {
        ...
        if (validatedData.name && validatedData.name !== existing.name) {
            const duplicate = await actionsRepository.isDuplicateName(validatedData.name, parsedId.actionId);
            if (duplicate)
    ...
    archiveAction: async ({ actionId }, meta) => {
    ...
    restoreAction: async ({ actionId }, meta) => {
        const parsedId = actionIdSchema.parse({ actionId });
        const existing = await actionsRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Action not found.");
    ...
    deleteAction: async ({ actionId }, meta) => {
        const parsedId = actionIdSchema.parse({ actionId });
        const existing = await actionsRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Action not found.");
    ...
    archiveActions: async (ids, meta) => {
        ...
        if (activeActions.length === 0) {
            throw new BadRequestException("No active actions found for the provided IDs.");
        }
        const nonSystemActive = activeActions.filter((a) => !a.isSystem);
        if (nonSystemActive.length === 0) {
            throw new BadRequestException("All selected actions are system actions and cannot be archived.");
        }
    ...
    restoreActions: async (ids, meta) => {
        const { actionIds } = actionIdsSchema.parse(ids);
        const existingActions = await actionsRepository.findManyByIds(actionIds);
        const inactiveActions = existingActions.filter((a) => a.deletedAt);
        if (inactiveActions.length === 0) {
            throw new BadRequestException("No archived actions found for the provided IDs.");
        }
    ...
    deleteActions: async (ids, meta) => {
        ...
        if (deletable.length === 0) {
            throw new BadRequestException("All selected actions are system actions and cannot be deleted.");
        }
