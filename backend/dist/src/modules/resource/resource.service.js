...
const actionsService = {
    ...
    //   }
    // },
    createResource: async (data, meta) => {
        const validatedData = createResourceSchema.parse(data);
        const key = generateKey(validatedData.name);
        // Check duplicate resource
        const existing = await resourcesRepository.findByNameAndTenant(validatedData.name);
        if (existing) {
            throw new BadRequestException(`Resource "${validatedData.name}" already exists.`);
        }
        try {
            const resource = await prisma.$transaction(async (tx) => {
    ...
    updateResourceById: async (id, data, meta) => {
        ...
        if (validatedData.name && validatedData.name !== existing.name) {
            const duplicate = await resourcesRepository.isDuplicateName(validatedData.name, parsedId.resourceId);
            if (duplicate)
                throw new BadRequestException("resource name already exists.");
        }
        let dataToUpdate = {};
    ...
    archiveResource: async ({ resourceId }, meta) => {
    ...
    restoreResource: async ({ resourceId }, meta) => {
        const parsedId = resourceIdSchema.parse({ resourceId });
        const existing = await resourcesRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Resource not found.");
    ...
    deleteResource: async ({ resourceId }, meta) => {
        const parsedId = resourceIdSchema.parse({ resourceId });
        const existing = await resourcesRepository.readOne(parsedId);
        if (!existing)
            throw new NotFoundException("Resource not found.");
    ...
    archiveResources: async (ids, meta) => {
        const { resourceIds } = resourceIdsSchema.parse(ids);
        const existingResponses = await resourcesRepository.findManyByIds(resourceIds);
        const activeResponses = existingResponses.filter((r) => !r.deletedAt);
        if (activeResponses.length === 0) {
            throw new BadRequestException("No active resources found for the provided IDs.");
        }
        const nonSystemActive = activeResponses.filter((r) => !r.isSystem);
        if (nonSystemActive.length === 0) {
            throw new BadRequestException("All selected resources are system resources and cannot be archived.");
        }
    ...
    restoreResources: async (ids, meta) => {
        const { resourceIds } = resourceIdsSchema.parse(ids);
        const existingResources = await resourcesRepository.findManyByIds(resourceIds);
        const inactiveResources = existingResources.filter((r) => r.deletedAt);
        if (inactiveResources.length === 0) {
            throw new BadRequestException("No archived resources found for the provided IDs.");
        }
    ...
    deleteResources: async (ids, meta) => {
        const { resourceIds } = resourceIdsSchema.parse(ids);
        const existingResources = await resourcesRepository.findManyByIds(resourceIds);
        ...
        if (deletable.length === 0) {
            throw new BadRequestException("All selected resources are system resources and cannot be deleted.");
        }
