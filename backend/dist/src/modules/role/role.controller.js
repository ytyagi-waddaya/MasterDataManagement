...
const roleController = {
    ...
    archiveRole: async (req, res) => {
        ...
        if (!roleId)
            throw new BadRequestException("Role Id is required");
        const role = await rolesService.archiveRole({ roleId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    restoreRole: async (req, res) => {
        ...
        if (!roleId)
            throw new BadRequestException("Role Id is required");
        const role = await rolesService.restoreRole({ roleId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteRole: async (req, res) => {
        ...
        if (!roleId)
            throw new BadRequestException("Role Id is required");
        const role = await rolesService.deleteRole({ roleId }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    archiveRoles: async (req, res) => {
        const { roleIds } = req.body;
        const roles = await rolesService.archiveRoles({ roleIds }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    restoreRoles: async (req, res) => {
        const { roleIds } = req.body;
        const roles = await rolesService.restoreRoles({ roleIds }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
    ...
    deleteRoles: async (req, res) => {
        const { roleIds } = req.body;
        const roles = await rolesService.deleteRoles({ roleIds }, {
            actorId: req.user?.id ?? null,
            ipAddress: req.ip,
            userAgent: req.get("user-agent") ?? null,
            performedBy: PerformedByType.USER,
