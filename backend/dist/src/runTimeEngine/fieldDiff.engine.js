import { Prisma, FieldType, } from "../../prisma/generated/client.js";
...
//   return fields;
// }
export function extractFieldsFromSchema(schema) {
    const keys = new Set();
    return schema.map((field) => {
        ...
        if (!field?.data?.type) {
            throw new BadRequestException(`Field "${field.meta.key}" is missing data.type`);
        }
        const widget = field.ui?.widget ?? "TEXT";
        if (!(widget in FieldType)) {
            throw new BadRequestException(`Invalid widget "${widget}" for field "${field.meta.key}"`);
        }
        return {
            key: field.meta.key,
            label: field.meta.label,
...
/* ======================================================
   FIELD DIFF ENGINE
===================================================== */
export async function applyFieldDiff({ tx, masterObjectId, previousSchemaId, newSchemaId, schemaJson, }) {
    ...
    for (const [order, field] of incomingFields.entries()) {
        ...
        const normalizedConfig = field.config && Object.keys(field.config).length > 0
            ? field.config
            : Prisma.JsonNull;
        if (!previous) {
        ...
        if (previous.dataType !== field.dataType) {
            throw new BadRequestException(`Cannot change data type of field "${field.key}"`);
        }
        if (previous.fieldType !== field.fieldType) {
            throw new BadRequestException(`Cannot change UI widget of field "${field.key}"`);
        }
        if (previous.category !== field.category) {
            throw new BadRequestException(`Cannot change category of field "${field.key}"`);
        }
        ...
        if (prevFormula &&
            ...
            throw new BadRequestException(`Cannot modify formula of calculated field "${field.key}"`);
        }
        /* -------- CARRY FORWARD -------- */
        await tx.fieldDefinition.create({
            data: {
...
===================================================== */
export async function publishSchemaWithDiff({ tx, masterObjectId, schemaJson, publish, createdById, }) {
...
//# sourceMappingURL=fieldDiff.engine.js.map