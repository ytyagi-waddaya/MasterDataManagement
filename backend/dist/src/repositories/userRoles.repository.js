...
const userRolesRepository = {
    ...
    assignMany: async (userIds, roleIds) => {
        const userIdArray = userIds.userIds;
        const roleIdArray = roleIds.roleIds;
        const data = userIdArray.flatMap((uid) => roleIdArray.map((rid) => ({
            userId: uid,
            roleId: rid,
        })));
    ...
    revokeMany: async (userIds, roleIds) => {
        const userIdArray = userIds.userIds;
        const roleIdArray = roleIds.roleIds;
...
//# sourceMappingURL=userRoles.repository.js.map