 ...
 */
async function processMessage(msgId, obj) {
...
export async function runWorkflowWorker() {
    ...
    while (running) {
        try {
            ...
            const res = await readGroup(config.CONSUMER_NAME, 10, 2000, config.WORKFLOW_STREAM);
            ...
            for (const [_stream, messages] of res) {
                ...
                for (const [id, keyvals] of messages) {
                    ...
                    const payloadObj = parseKeyVals(keyvals);
                    try {
                        await processMessage(id, payloadObj);
                        await ack(id);
                        logger.info("[workflow.worker] acked", { id });
                    }
                    catch (err) {
                        ...
                        try {
                            await moveToDLQ(id, payloadObj, String(err?.message ?? err));
                        }
                        catch (dlqErr) {
                            logger.error("[workflow.worker] moveToDLQ FAILED", {
                                id,
