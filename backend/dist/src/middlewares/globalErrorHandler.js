 ...
 */
function sendToMonitoring(err, meta) {
    // Example:
...
export const globalErrorHandler = (err, req, res, _next) => {
    ...
    if (err instanceof AppError) {
        ...
        appErr = new AppError(err.message || "Unexpected error", HTTPSTATUS.INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR", undefined, false, err);
    }
    else {
        // Non-Error thrown (e.g., string or other)
        appErr = new AppError("Unknown error", HTTPSTATUS.INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR", { raw: err }, false);
    }
    ...
    const meta = {
        ...
        retryAfterSeconds: appErr.retryAfterSeconds ?? undefined,
    ...
    if (appErr.isOperational) {
        ...
        try {
            ...
            logger.debug("Monitoring integration failed", { err: monitorErr.message ?? monitorErr });
        }
    }
    ...
    const retryAfter = appErr.retryAfterSeconds;
