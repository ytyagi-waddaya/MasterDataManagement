...
export function autoAuthorize() {
    return async (req, res, next) => {
        try {
            ...
            if (rolePermissions.some((rp) => rp.accessLevel === "FULL")) {
                return next();
            }
            ...
            for (const rp of rolePermissions) {
                ...
                if (rp.accessLevel === "CONDITIONAL" && rp.conditions) {
                    try {
                        if (evaluateGroup(rp.conditions, context)) {
                            return next();
                        }
                    }
                    catch (err) {
                ...
                if (rp.accessLevel === "CONDITIONAL" &&
                    ...
                    try {
                        if (evaluateGroup(permission.conditions, context)) {
                            return next();
                        }
                    }
                    catch (err) {
...
=============================================================== */
function buildContext(req, dbUser, actor) {
    return {
        user: dbUser,
        actor,
        params: req.params,
...
function evaluateGroup(node, ctx) {
    ...
    if (node.op === "AND") {
        return node.rules.every((r) => evaluateGroup(r, ctx));
    }
    if (node.op === "OR") {
        return node.rules.some((r) => evaluateGroup(r, ctx));
    }
    return false;
}
/* ============================================================
...
=============================================================== */
function runOperator(op, left, right) {
    switch (op) {
