...
const actionsController = {
  ...
  updateActionById: async (req: Request, res: Response) => {
    const actionId = req.params?.actionId;
  ...
  archiveAction: async (req: Request, res: Response) => {
    const actionId = req.params?.actionId;
    ...
    const action = await actionsService.archiveAction(
      { actionId },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
  ...
  restoreAction: async (req: Request, res: Response) => {
    const actionId = req.params?.actionId;
    ...
    const action = await actionsService.restoreAction(
      { actionId },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
  ...
  deleteAction: async (req: Request, res: Response) => {
    ...
    const action = await actionsService.deleteAction(
      { actionId },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
  ...
  archiveActions: async (req: Request, res: Response) => {
    const { actionIds } = req.body;

    const actions = await actionsService.archiveActions(
      { actionIds },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
  ...
  restoreActions: async (req: Request, res: Response) => {
    const { actionIds } = req.body;

    const actions = await actionsService.restoreActions(
      { actionIds },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
  ...
  deleteActions: async (req: Request, res: Response) => {
    const { actionIds } = req.body;

    const actions = await actionsService.deleteActions(
      { actionIds },
      {
        actorId: req.user?.id ?? null,
        ipAddress: req.ip,
