...
const usersService = {
  createUser: async (data: CreateUserInput, meta?: ActorMeta) => {
  ...
  createUsers: async (data: CreateUserInput[], meta?: ActorMeta) => {
    if (!data.length) {
    ...
    if (existingEmails.length > 0) {
      throw new BadRequestException(
        `Users already exist: ${existingEmails.join(", ")}`
      );
    }
  ...
  getUsers: async (options?: Partial<userFilterInput>) => {
  ...
  updateUser: async (id: UserId, data: UpdateUserInput, meta?: ActorMeta) => {
  ...
  archiveUser: async ({ userId }: UserId, meta?: ActorMeta) => {
    const parsedUserId = userIdSchema.parse({ userId });
    ...
    if (!archived) throw new NotFoundException("User not found.");
  ...
  restoreUser: async ({ userId }: UserId, meta?: ActorMeta) => {
    const parsedUserId = userIdSchema.parse({ userId });
  ...
  deleteUser: async ({ userId }: UserId, meta?: ActorMeta) => {
    const parsedUserId = userIdSchema.parse({ userId });
    if (!parsedUserId) throw new BadRequestException("User ID is required.");
  ...
  archiveUsers: async (ids: UserIds, meta?: ActorMeta) => {
    ...
    if (activeUsers.length === 0)
      throw new BadRequestException(
        "No active users found for the provided IDs."
      );
    ...
    if (nonSystem.length === 0)
      throw new BadRequestException(
        "All selected users are system/service accounts and cannot be archived."
      );
  ...
  restoreUsers: async (ids: UserIds, meta?: ActorMeta) => {
    const { userIds } = userIdsSchema.parse(ids);
    ...
    if (archivedUsers.length === 0)
      throw new BadRequestException(
        "No archived users found for the provided IDs."
      );
  ...
  deleteUsers: async (ids: UserIds, meta?: ActorMeta) => {
    const { userIds } = userIdsSchema.parse(ids);
    ...
    if (deletable.length === 0)
      throw new BadRequestException(
        "All selected users are system/service accounts and cannot be deleted."
      );
    ...
    const result = await prisma.$transaction(async (tx) => {
      const deletedRecords = await Promise.all(
        idsToDelete.map((id) => tx.user.delete({ where: { id } }))
      );

      await Promise.all(
        deletedRecords.map((u) =>
          tx.auditLog.create({
            data: {
              userId: meta?.actorId ?? null,
              entity: AuditEntity.USER,
