...
const roleService = {
  createRole: async (data: CreateRoleInput, meta?: ActorMeta) => {
    const validatedData = createRoleSchema.parse(data);
    const key = generateKey(validatedData.name);
    const existing = await roleRepository.findByNameAndTenant(
      validatedData.name
    );

    if (existing) {
      throw new BadRequestException(
        `Role "${validatedData.name}" already exists.`
      );
    }

    const toCreate: Prisma.RoleCreateInput = {
  ...
  updateRoleById: async (
    ...
    if (validatedData.name && validatedData.name !== existing.name) {
      const duplicate = await roleRepository.isDuplicateName(
        validatedData.name,
        parsedId.roleId
      );
    ...
    let dataToUpdate: Prisma.RoleUpdateInput = {};
  ...
  archiveRole: async ({ roleId }: RoleId, meta?: ActorMeta) => {
    ...
    if (!archived) throw new NotFoundException("Role not found.");
  ...
  restoreRole: async ({ roleId }: RoleId, meta?: ActorMeta) => {
    const parsedId = roleIdSchema.parse({roleId});
    ...
    if (!restored) throw new NotFoundException("Role not found.");
  ...
  deleteRole: async ({ roleId }: RoleId, meta?: ActorMeta) => {
    const parsedId = roleIdSchema.parse({roleId});
    const existing = await roleRepository.readOne(parsedId);
    if (!existing) throw new NotFoundException("Role not found.");
  ...
  archiveRoles: async (ids: RoleIds, meta?: ActorMeta) => {
    ...
    if (activeRoles.length === 0)
      throw new BadRequestException(
        "No active roles found for the provided IDs."
      );

    const nonSystemActive = activeRoles.filter((r) => !r.isSystem);
    if (nonSystemActive.length === 0) {
      throw new BadRequestException(
        "All selected roles are system roles and cannot be archived."
      );
    }
  ...
  restoreRoles: async (ids: RoleIds, meta?: ActorMeta) => {
    const { roleIds } = roleIdsSchema.parse(ids);
    const existingRoles = await roleRepository.findManyByIds(roleIds);
    ...
    if (inactiveRoles.length === 0) {
      throw new BadRequestException(
        "No archived roles found for the provided IDs."
      );
    }
  ...
  deleteRoles: async (ids: RoleIds, meta?: ActorMeta) => {
    const { roleIds } = roleIdsSchema.parse(ids);
    const existingRoles = await roleRepository.findManyByIds(roleIds);
    ...
    if (deletable.length === 0) {
      throw new BadRequestException(
        "All selected roles are system roles and cannot be deleted."
      );
    }
