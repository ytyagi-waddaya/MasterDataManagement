...
const authService = {
  /** USER LOGIN */
  login: async (data: LoginInput, res: Response, meta?: ActorMeta) => {
    ...
    const accessToken = generateJwtToken({
      ...
      type: user.type,
      status: user.status,
      departmentIds: user.department.map(
        (d) => d.department.id
      ),
  ...
  refreshAccessToken: async (
    ...
    const accessToken = generateJwtToken({
      ...
      departmentIds: existingToken.user.department.map(
        (d) => d.department.id
      ),
  ...
  logout: async (refreshToken: string, res: Response, meta?: ActorMeta) => {
    const tokenHash = hashToken(refreshToken);
