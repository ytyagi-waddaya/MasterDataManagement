...
// import { adapterPub, adapterSub, appSub, USE_REDIS } from "../redis/client.js";
...
//     try {
//       await appSub.psubscribe("notifications:user:*");
...
//   (async function subscribeEventBus() {
//   try {
//     await appSub.subscribe("erp:events");
//     logger.info("[socket] subscribed to erp:events");
//   } catch (err) {
//     logger.error("[socket] subscribe erp:events failed", err);
//   }

//   appSub.on("message", (_channel: string, message: string) => {
//     try {
...
export function createSocketServer(app: express.Express) {
  ...
  /* ─────────────────────────────────────────────
     REDIS ADAPTER (only when enabled)
  ───────────────────────────────────────────── */
  if (USE_REDIS && adapterPub && adapterSub) {
    io.adapter(createAdapter(adapterPub, adapterSub));
    logger.info("[socket] redis adapter enabled");
  } else {
    logger.warn("[socket] redis adapter disabled");
  }
  ...
  /* ─────────────────────────────────────────────
     REDIS SUBSCRIPTIONS (ONLY WHEN ENABLED)
  ───────────────────────────────────────────── */
  if (USE_REDIS && appSub) {
    ...
    (async function subscribeUserChannels() {
      try {
        await appSub.psubscribe("notifications:user:*");
        logger.info("[socket] psubscribed notifications:user:*");
      } catch (err) {
        logger.error(
          "[socket] psubscribe notifications:user:* failed",
          err
        );
      }

      appSub.on("pmessage", (_p, channel, message) => {
        ...
        try {
          ...
          logger.error(
            "[socket] redis message parse error",
            err
          );
        }
      });
    ...
    (async function subscribeEventBus() {
      try {
        await appSub.subscribe("erp:events");
        logger.info("[socket] subscribed erp:events");
      } catch (err) {
        logger.error(
          "[socket] subscribe erp:events failed",
          err
        );
      }

      appSub.on("message", (_c, message) => {
        try {
          const evt = JSON.parse(message);
          broadcast(io, evt);
        } catch (err) {
          logger.error(
            "[socket] erp:events parse error",
            err
          );
        }
      });
    ...
    logger.warn(
      "[socket] redis subscriptions disabled"
    );
  }
...
function broadcast(io: Server, evt: any) {
  ...
  if (evt.payload?.resource) {
    io.to(`resource:${evt.payload.resource}`).emit(
      evt.event,
      evt
    );
  }
