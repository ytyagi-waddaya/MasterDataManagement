...
import { HistoryAction, Prisma } from "../../prisma/generated/client.js";
...
async function enforceTransitionPermission(
  userId: string,
  transition: any,
...
async function handleApprovalTransition({
  ...
  if (!input.action) throw new BadRequestException("Approval action required");

  const config = transition.approvalConfig;
  if (!config) throw new BadRequestException("approvalConfig missing");
...
function initializeApprovalMeta(config: any) {
  if (config.mode === "SEQUENTIAL") {
    return {
      mode: "SEQUENTIAL",
      currentLevel: 0,
      approvals: config.levels.map((_: any, i: number) => ({
        level: i,
        status: "PENDING",
        userId: null,
      })),
  ...
  if (config.mode === "PARALLEL") {
    return {
      ...
      approvals: config.approvers.map((a: any) => ({
...
async function handleSequentialApproval({
  tx,
  instance,
  approvalMeta,
  actorId,
...
async function handleParallelApproval({
  tx,
  instance,
  approvalMeta,
  actorId,
  ...
  const entry = approvalMeta.approvals.find(
    (a: any) => a.userId === actorId || a.roleId
  );
  ...
  const approvedCount = approvalMeta.approvals.filter(
    (a: any) => a.status === "APPROVED"
  ).length;

  const completed =
    approvalMeta.strategy === "ANY"
      ? approvedCount >= 1
...
function evaluateCondition(condition: any, resourceId: string): boolean {
