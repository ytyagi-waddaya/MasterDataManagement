...
// import { USE_REDIS, connectRedis, shutdownRedis } from "./redis/client.js";
...
async function gracefulShutdown(reason: string) {
  ...
  // Redis disconnect (only if enabled)
  if (USE_REDIS) {
    try {
      await shutdownRedis();
...
async function start() {
  try {
    // connect redis only when enabled
    if (USE_REDIS) {
      await connectRedis();
    } else {
      logger.warn("[api] redis disabled (USE_REDIS=false)");
    }
