...
import { RuntimeContext, RuntimeResult } from "./types.js";

export class SchemaRuntimeEngine {
  static execute(ctx: RuntimeContext): RuntimeResult {
    const compiled = SchemaCompiler.compile(ctx.fields);
    const values: Record<string, any> = { ...ctx.values };

    const executed: RuntimeResult["fields"] = [];
    ...
    /* --------------------------------------------------
       2️⃣ VALIDATION
    -------------------------------------------------- */
    const validationErrors = ValidationEngine.validate(
      ctx.validationRules,
      ctx,
      values
    );
    ...
    for (const field of compiled.orderedFields) {
      ...
      /* ---------- VISIBILITY ---------- */
      const visible = VisibilityEvaluator.isVisible(
        field.visibility,
        ctx,
        values
      );
      ...
      /* ---------- WRITE PERMISSION ---------- */
      const canWrite =
        visible && PermissionEvaluator.canWrite(field, ctx, values);
      /* ---------- VALIDATION ERRORS ---------- */
      const fieldValidationErrors = validationErrors[fieldKey] ?? [];

      const blockingErrors = fieldValidationErrors.filter(
        (e) => e.severity === "ERROR"
      );
      ...
      executed.push({
        ...
        visible,
        required:
          field.isRequired && visible && ctx.mode === "SUBMIT",
        readOnly: !canWrite,
