...
import { Type, Hash, Calendar, CheckSquare, List, File, Database, CheckCircle, Star, ThumbsUp } from "lucide-react";
...
export function GeneralTab({
  ...
  const fieldIcons: Record<EditorFieldType, React.ComponentType<any>> = {
    text: Type, textarea: Type, rich_text: Type, email: Type, phone: Type, url: Type, password: Type,
    number: Hash, currency: Hash, percentage: Hash,
    boolean: CheckSquare, consent: CheckSquare,
    select: List, multi_select: List, radio: List, rating: Star, scale: ThumbsUp, matrix: Hash,
    date: Calendar, datetime: Calendar,
    file: File, image: File,
    reference: Database, multi_reference: Database, user: Database, role: Database, status: CheckCircle,
    approval: CheckCircle, checklist: List, captcha: CheckCircle,
    json: Database,
  };
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <select
          value={f.layout.span}
          onChange={(e) => update({ 
            layout: { ...f.layout, span: Number(e.target.value) as LayoutSpan } 
          })}
          className="w-full text-sm px-3 py-2 rounded border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900"
        >
          {[3, 4, 6, 8, 12].map((span) => (
      ...
      {/* Placeholder */}
      {["text", "textarea", "email", "phone", "url", "number", "currency", "percentage"].includes(fieldType) && (
        <div>
          <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">
            Placeholder
          </label>
      ...
      {/* Default Value */}
      {["text", "textarea", "email", "phone", "url", "number", "currency", "percentage", "boolean", "select", "radio"].includes(fieldType) && (
        <div>
          <label className="text-xs font-medium text-gray-700 dark:text-gray-300 mb-1 block">
            Default Value
          </label>
