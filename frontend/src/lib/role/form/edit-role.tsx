...
import { updateRoleSchema, UpdateRoleInput } from "../schema/role-schema";
...
export const EditRoleForm: React.FC<Props> = ({
  initialValues,
  onSubmit,
  onClose,
  disabled,
  ...
  const { form, errors, touched, setValue, onBlur, validateForm, reset } =
    useZodForm(updateRoleSchema, initialValues);
      ...
      <div className="flex justify-between items-center rounded-md border p-3 bg-muted/30">
        <div>
          ...
          <p className=" flex items-center gap-1 text-xs text-muted-foreground">
            ...
            {form.isSystem
              ? "This is a system role and cannot be changed."
              : "Enable if this role should be a system role."}
          </p>
        </div>
        <Switch
          id="isSystem"
